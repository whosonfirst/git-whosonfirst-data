...
```
import wof.validate
ok = validate.validate_files(repo, files)
```
...
```
import wof.meta
modified, created = meta.update_metafiles(repo, files)
```
...
modified, created = concordances.update_concordances(repo, files)
```

## syncing to S3

```
$> cat .changed-* | sort | uniq | grep geojson | /path/to/git-whosonfirst-data/bin/wof/s3.py -r `pwd` -c /path/to/git-whosonfirst-data/hooks/hooks.cfg -t /path/to/wof-sync-files
...
pid = s3.sync_files(root, files, sync_tool, cfg)
```

## syncing to ES (Elasticsearch)

```
$> cat .changed-* | sort | uniq | grep geojson | /path/to/git-whosonfirst-data/bin/wof/es.py -r `pwd` -c /path/to/git-whosonfirst-data/hooks/hooks.cfg -t /path/to/wof-es-index-filelist
...
pid = es.sync_files(root, files, sync_tool, cfg)
```
